...
/// Creates an <code>EditController</code> with the provided session. The session you pass here will be saved immediately and will be automatically saved when any changes are made.
/// warning:
...
@required
/// This will be called when the Done button is pressed. You should push on a new controller to the navigation stack or dismiss the controller here.
/// attention:
...
///   </li>
///   <li>
///     Programmatically make modifications to a session like set filters, crop rects, segment trim times, etc.
///   </li>
/// </ul>
...
/// For example, setting the primaryFilter of a video session:
/// \code
...
/// This notification will be posted to the default NotificationCenter after the <code>savedSessions</code> array has been populated. This will happen shortly after the application starts.
...
@property (nonatomic, readonly, copy) NSDate * _Nullable dateExported;
/// Indicates if the export was completed.
/// If the sessions dateModified is an older date than the videos dateExported, this will be false. In other words, if you export the video this will become true but if you then make modifications to the session this will flip back to false. The exportedVideoURL file must also exist for this to be true.
@property (nonatomic, readonly) BOOL isExported;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
